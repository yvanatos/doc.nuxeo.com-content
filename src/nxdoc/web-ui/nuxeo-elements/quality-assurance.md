...
Testing your custom elements is paramount if you want to have reliable and easy to maintain components. We are using Web Test Runner, from [Open Webcomponents](https://open-wc.org/docs/testing/testing-package/). It is a handy library that allows you to **unit test** your elements.
...
All the elements to be tested, or to support testing directly, should be declared as a `fixture`. Imagine we want to unit test the `nuxeo-collections` element. We must declare the test fixture as follows:
...
You can then run your tests using `npm run test`. For more examples on testing custom elements, please check our repositories:
- [nuxeo-elements](https://github.com/nuxeo/nuxeo-elements/tree/maintenance-3.0.x/testing-helpers)
