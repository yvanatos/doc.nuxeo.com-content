...
```shell
docker run --name nuxeo -p 8080:8080 docker.packages.nuxeo.com/nuxeo/nuxeo:latest
```
...
```shell
docker run -it --rm docker.packages.nuxeo.com/nuxeo/nuxeo:latest bash
```
...
docker run --name nuxeo \
  ...
  docker.packages.nuxeo.com/nuxeo/nuxeo:latest
```
